...
- (NSString *)getCurrentDay {
    NSString *currentDate = [self getDayWithYYMMDD];
    if (currentDate.length>2) {
        NSString *day = [currentDate substringWithRange:NSMakeRange(currentDate.length-2, 2)];
        return day;
    }
    return nil;
}
...
- (NSString *)ruleMoneyType:(NSString *)money{
    ...
    if ([[HRNetworkTool shareTool] stringUserfulContent:money]) {
...
- (NSDictionary *)convertSafeDictWithDic:(NSDictionary *)dic {
    if ([[HRNetworkTool shareTool] isCheckData:dic]) {
...
//    NSDate *date = [NSDate dateWithString:timeStr format:formatter];
//    self.formatter.dateFormat = toFormatter;
//    NSString *convertStr = [self.formatter stringFromDate:date];
//    return convertStr;
//}
...
//    else{
//    }
    timeStr = [NSString stringWithFormat:@"%zd",second];
