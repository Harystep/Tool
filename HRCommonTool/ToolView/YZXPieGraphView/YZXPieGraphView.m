...
- (instancetype)initWithFrame:(CGRect)frame withTitleData:(NSArray *)titleArr withPercentageData:(NSArray *)precentageArr withMoneyArr:(NSArray *)MoneyArr withColors:(NSArray *)colorArr
{
    ...
    if (self.precentageArr.count == 0) {
        ...
        [self.precentageArr enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            if (weak_self.titleArr.count != 0) {
                ...
                NSString * moneyStr = weak_self.MoneyArr[idx];
                if (moneyStr.floatValue >10000) {
                    
                    NSString * moneey = [NSString stringWithFormat:@"%.2f万",moneyStr.floatValue/10000];
                    title = [NSString stringWithFormat:@"%@\n%@\n¥%@元",weak_self.titleArr[idx],weak_self.precentageArr[idx],moneey];
                }else{
                    title = [NSString stringWithFormat:@"%@\n%@\n¥%@元",weak_self.titleArr[idx],weak_self.precentageArr[idx],weak_self.MoneyArr[idx]];
                }
                ...
                if (!weak_self.hideTitlt) {
                    ...
                    //画线
                    CGContextSetStrokeColorWithColor(context, HEXCOLOR(0x999999).CGColor);
                    CGContextMoveToPoint(context, start_x+9, start_y);
                    CGContextAddLineToPoint(context, end_x+9, end_y);
                    CGContextSetLineWidth(context, 1);
                    CGContextDrawPath(context, kCGPathStroke);
                    
                    //添加标题（左右为竖向显示，上下为横向显示）
                    if (angle < M_PI_2 && angle > -M_PI_2) {//右
                        [title drawInRect:CGRectMake(end_x+12.5, end_y - titleSize.height / 2.0, titleSize.width, titleSize.height) withAttributes:@{NSFontAttributeName:font,NSForegroundColorAttributeName:titleColor}];
