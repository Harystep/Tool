...
@property (nonatomic, copy) BRStringCanelBlcok cancelBlock;
// 单列选中的项
...
+ (void)showStringPickerWithTitle:(NSString *)title
                  ...
                  defaultSelValue:(id)defaultSelValue
                     isAutoSelect:(BOOL)isAutoSelect
                      resultBlock:(BRStringResultBlock)resultBlock cancelBlcok:(BRStringCanelBlcok)cancelBlock{
    if (dataSource == nil || dataSource.count == 0) {
        return;
    }
    BRStringPickerView *strPickerView = [[BRStringPickerView alloc]initWithTitle:title dataSource:dataSource defaultSelValue:defaultSelValue isAutoSelect:isAutoSelect resultBlock:resultBlock cancelBlcok:cancelBlock];
...
+ (void)showStringPickerWithTitle:(NSString *)title
    ...
    BRStringPickerView *strPickerView = [[BRStringPickerView alloc]initWithTitle:title dataSource:dataSource defaultSelValue:defaultSelValue isAutoSelect:isAutoSelect resultBlock:resultBlock cancelBlcok:nil];
    [strPickerView showWithAnimation:YES];
}
...
- (instancetype)initWithTitle:(NSString *)title
              ...
              defaultSelValue:(id)defaultSelValue
                 isAutoSelect:(BOOL)isAutoSelect
                  resultBlock:(BRStringResultBlock)resultBlock
                  cancelBlcok:(BRStringCanelBlcok)cancelBlock{
    if (self = [super init]) {
        self.title = title;
