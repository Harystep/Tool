...
#import "HRPieItemsView.h"
#import "HRPieForeBGView.h"
#import "HRShowInfoView.h"
#define k_COLOR_STOCK @[[UIColor colorWithRed:244/255.0 green:161/255.0 blue:100/255.0 alpha:1], [UIColor colorWithRed:254/255.0 green:224/255.0 blue:90/255.0 alpha:1],[UIColor colorWithRed:240/255.0 green:58/255.0 blue:63/255.0 alpha:1],[UIColor colorWithRed:147/255.0 green:111/255.0 blue:255/255.0 alpha:1]]

@interface HRPieChartView()

@property (nonatomic,strong) HRPieForeBGView *pieForeView;


@property (assign , nonatomic) double  allValueCount;
...
@property (nonatomic,strong) HRShowInfoView *showInfoView;
@end


@implementation HRPieChartView
...
-(instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self = [super initWithFrame:CGRectMake(kRealScale(20), 10, 125, 125)]) {
        _chartArcLength = 8.0;
    }
    return self;
}
...
-(void)drawRect:(CGRect)rect{
    ...
    if (_descArr.count==_valueArr.count) {
...
- (void)countAllValue{
    ...
    for (NSString *obj in _valueArr) {
        ...
        _allValueCount += obj.doubleValue;
...
- (void)countAllAngleDataArr{
    ...
    for (NSString *obj in _valueArr) {
        if (obj.doubleValue > 0.0) {
            [_angleArr addObject:[NSNumber numberWithDouble:obj.doubleValue*M_PI * 2/_allValueCount]];
        } else {
            [_angleArr addObject:@"0.0"];
        }
...
-(void)showAnimation {
    ...
    if (_valueArr.count>0) {
        ...
        self.chartOrigin = P_M(wid / 2, wid / 2);
        ...
        for (NSInteger i = 0; i<_countPreAngeleArr.count-1; i++) {
            ...
            HRPieItemsView *itemsView = [[HRPieItemsView alloc] initWithFrame:CGRectMake(wid/4.0, wid/4.0, wid/2.0, wid/2.0) andBeginAngle:[_countPreAngeleArr[i] floatValue] andEndAngle:[_countPreAngeleArr[i+1] floatValue] andFillColor:colors[i%colors.count]];
...
//            itemsView.center = CGPointMake(self.frame.size.width/2, 10+wid/2);
        ...
        _pieForeView = [[HRPieForeBGView alloc] initWithFrame:CGRectMake(0, 0, wid,  wid)];
        
//        _pieForeView.center = CGPointMake(self.frame.size.width/2, 10+wid/2);
        NSLog(@"_pieForeView--->%@", _pieForeView);
        _pieForeView.backgroundColor = [UIColor clearColor];
//        if (_showInfoView==nil) {
//            _showInfoView = [[HRShowInfoView alloc] init];
...
- (void)judgeWhitchOneIsNowAngle:(CGFloat )angel andShowPoint:(CGPoint)p{
    ...
    for (NSInteger i = 0; i<_countPreAngeleArr.count-1; i++) {
        ...
        if ([_countPreAngeleArr[i+1] floatValue]>=angel) {
            ...
            HRPieItemsView *itemsView = _layersArr[i];
            
            HRPieItemsView *saveItems = _layersArr[_saveIndex];
