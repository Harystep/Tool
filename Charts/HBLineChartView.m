...
//    _totalWidth = (self.gapWidth+(self.barWidth+self.gapWidth)*self.xValuesArr.count)>(kScreenWidth-2*kHRMarginX)?(self.gapWidth+(self.barWidth+self.gapWidth)*self.xValuesArr.count):(kScreenWidth-2*kHRMarginX);
    ...
    self.scrollView.contentSize = CGSizeMake(45+_totalWidth, 0);
    self.contentView.frame = CGRectMake(45,30, _totalWidth,_totalHeight);
    [self drawLineChart];
}
//开始绘制图表
-(void)drawLineChart{
...
-(void)addYAxisLabs{
    for (int i = self.yAxisCount ; i >= 0; i--) {
        ...
        lab.frame = CGRectMake(-2, (self.yAxisCount-i)*(_totalHeight/self.yAxisCount)-10, -45, 20);
...
//            lab.text = [NSString stringWithFormat:@"%.2f",showNumber];
//            lab.text = value;
//            lab.text = [NSString stringWithFormat:@"%@%.2f%@",self.unit,showNumber,[self generateUnitWithType:type unit:@""]];
    
            lab.text = [NSString stringWithFormat:@"%@%@",self.xValueUnit,[self generateRealValueWithValue:[self.yValuesArr[i] floatValue] length:2]];
        } else {
//            lab.text = value;
//            lab.text = [NSString stringWithFormat:@"%.2f",showNumber];
//            lab.text = [NSString stringWithFormat:@"%.2f%@%@",showNumber,[self generateUnitWithType:type unit:@""],self.unit];
